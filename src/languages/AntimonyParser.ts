interface Compartment {
  ...
  export function parseAntimonyModel(antimonyModel: string): AntimonyModel {
    ...
    let currentSection: 'compartments' | 'species' | 'reactions' | 'declarations' | null = null;
    ...
    lines.forEach((line) => {
      ...
      if (currentSection === 'compartments') {
        const match = line.match(/compartment (.+);/);
        if (match) {
        ...
        const match = line.match(/(var (.+)|const (.+))/);
        if (match) {
